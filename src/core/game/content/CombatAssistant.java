...
public class CombatAssistant {
    ...
    public void attackNpc(int i) {
        if (Server.npcHandler.npcs[i] != null) {
            ...
            if (c.attackTimer <= 0) {
                ...
                c.attackTimer = getAttackDelay(c.getEquipment().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
                c.specAccuracy = 1.0;
                c.specDamage = 1.0;
                if (!c.usingMagic) {
                ...
                if (!c.usingMagic) {
                    c.startAnimation(getWepAnim(c.getEquipment().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()));
                } else {
    ...
    public void delayedHit(int i) { // npc hit delay
        if (Server.npcHandler.npcs[i] != null) {
            ...
            if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
                ...
                if (c.fightMode == 3) {
                    c.getPA().addSkillXP((damage * GameConstants.RANGE_EXP_RATE / 3), 4);
                    c.getPA().addSkillXP((damage * GameConstants.RANGE_EXP_RATE / 3), 1);
                    c.getPA().addSkillXP((damage * GameConstants.RANGE_EXP_RATE / 3), 3);
                ...
                c.getPA().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage * GameConstants.MAGIC_EXP_RATE), 6);
                c.getPA().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage * GameConstants.MAGIC_EXP_RATE / 3), 3);
                c.getPA().refreshSkill(3);
        ...
        if (c.bowSpecShot >= 2) {
            c.bowSpecShot = 0;
            // c.attackTimer =
            // getAttackDelay(c.getEquipment().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
        }
        if (c.bowSpecShot == 1) {
            fireProjectileNpc();
            c.hitDelay = 2;
    ...
    public void applyPlayerMeleeDamage(int i, int damageMask) {
        ...
        if (Misc.random(o.getCombat().calculateMeleeDefence()) > Misc.random(calculateMeleeAttack())) {
            damage = 0;
            c.bonusAttack = 0;
        ...
        if (o.prayerActive[18] && System.currentTimeMillis() - o.protMeleeDelay > 1500) { // if
            // prayer
            // active
            // reduce
            // damage
    ...
    public void activateSpecial(int weapon, int i) {
        ...
        switch (weapon) {
            ...
            case 1434: // mace
                ...
                c.hitDelay = getHitDelay(c.getEquipment().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()) + 1;
                c.specDamage = 1.35;
                c.specAccuracy = 1.15;
                break;
    ...
    public boolean checkMagicReqs(int spell) {
        ...
        if (c.usingMagic && staffRequired > 0 && Config.RUNES_REQUIRED) { // staff
            ...
            if (c.playerEquipment[c.playerWeapon] != staffRequired) {
                c.sendMessage("You need a " + c.getEquipment().getItemName(staffRequired).toLowerCase() + " to cast this spell.");
                return false;
            }
        }
