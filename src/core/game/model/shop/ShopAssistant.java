...
public class ShopAssistant {
	...
	**/	
	public void openShop(int ShopID){		
		...
		c.getActionSender().textOnInterface(ShopHandler.ShopName[ShopID], 3901);
	}
	...
	**/
	...
	public void buyFromShopPrice(int removeId, int removeSlot){
		...
		String ShopAdd = "";
		if (c.myShopId >= 17) {
			c.sendMessage(c.getEquipment().getItemName(removeId)+": currently costs " + getSpecialItemValue(removeId) + " points.");
			return;
		}
		if (c.myShopId == 15) {
		...
		c.sendMessage(c.getEquipment().getItemName(removeId)+": currently costs "+ShopValue+" coins"+ShopAdd);
	}
	...
	**/
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		if (IsIn == false) {
			c.sendMessage("You can't sell "+c.getEquipment().getItemName(removeId).toLowerCase()+" to this store.");
		} else {
			...
			c.sendMessage(c.getEquipment().getItemName(removeId)+": shop will buy for "+ShopValue+" coins"+ShopAdd);
		}
	}
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			if (ShopHandler.ShopSModifier[c.myShopId] > 1) {
				...
				if (IsIn == false) {
					c.sendMessage("You can't sell "+c.getEquipment().getItemName(itemID).toLowerCase()+" to this store.");
					return false;
				}
			}
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		public void setupSkillCapes(int capes, int capes2) {
			synchronized(c) {
				...
				c.myShopId = 14;
				c.getActionSender().sendFrame248(3824, 3822);
				c.getActionSender().textOnInterface("Skillcape Shop", 3901);
		...
		public void openVoid() {
			/*synchronized(c) {
				...
				c.myShopId = 15;
				c.getActionSender().sendFrame248(3824, 3822);
				c.getActionSender().textOnInterface("Void Recovery", 3901);
