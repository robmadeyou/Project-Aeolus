...
public class MagicRequirements extends MagicData {
	...
	public static boolean hasRunes(Player c, int[] runes, int[] amount) {
		for (int i = 0; i < runes.length; i++) {
			if (c.getInventory().playerHasItem(runes[i], amount[i])
	...
	public static void deleteRunes(Player c, int[] runes, int[] amount) {
		if (c.getRights().lessOrEqual(Rights.MODERATOR)) {
			for (int i = 0; i < runes.length; i++) {
				c.getInventory().deleteItem(runes[i],
						c.getEquipment().getItemSlot(runes[i]), amount[i]);
			}
		}
	}
	...
	public static boolean checkMagicReqs(Player c, int spell) {
		if (c.usingMagic && Configuration.RUNES_REQUIRED) { // check for runes
			if ((!c.getInventory().playerHasItem(c.MAGIC_SPELLS[spell][8],
					...
					|| (!c.getInventory().playerHasItem(c.MAGIC_SPELLS[spell][10],
							c.MAGIC_SPELLS[spell][11]) && !wearingStaff(c,
							c.MAGIC_SPELLS[spell][10]))
					|| (!c.getInventory().playerHasItem(c.MAGIC_SPELLS[spell][12],
							c.MAGIC_SPELLS[spell][13]) && !wearingStaff(c,
							c.MAGIC_SPELLS[spell][12]))
					|| (!c.getInventory().playerHasItem(c.MAGIC_SPELLS[spell][14],
		...
		if (c.usingMagic && staffRequired > 0 && Configuration.RUNES_REQUIRED) { // staff
			...
			if (c.playerEquipment[c.playerWeapon] != staffRequired) {
				c.sendMessage("You need a "
						+ c.getEquipment().getItemName(staffRequired).toLowerCase()
						+ " to cast this spell.");
				return false;
			}
		}
			...
			if (c.MAGIC_SPELLS[spell][8] > 0) { // deleting runes
				if (!wearingStaff(c, c.MAGIC_SPELLS[spell][8]))
					c.getInventory().deleteItem(c.MAGIC_SPELLS[spell][8],
							c.getEquipment().getItemSlot(c.MAGIC_SPELLS[spell][8]),
